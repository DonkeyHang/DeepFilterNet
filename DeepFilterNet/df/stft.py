...
def vorbis_window(n):
    ...
    # n_double = np.tensor(n, dtype=np.float64)
    window = np.sin((np.pi / 2.0) * np.power(np.sin(indices / n * np.pi), 2.0))
...
class STFT:
    ...
    def __init__(self, framesize: int, *, hopsize: Union[int, None] = None, padsize: int = 0, center: bool = False, window: Union[bool, str, None] = True):
        ...
        self.window_vorbis    = vorbis_window(self.framesize)

    def get_window(self):
        return self.window_vorbis
    ...
    def istft(self, dfts: ArrayLike) -> NDArray:
        ...
        gain = self.hopsize / np.sum(np.square(self.window_vorbis))
    ...
    def fft(self, data: ArrayLike) -> NDArray:
        ...
        data = np.atleast_2d(data) * self.window_vorbis
